...
export default function TechStack() {
  return (
    <>
      <SEO
        title="Technology Stack | Codeneir"
        description="Explore the technologies Codeneir uses to build secure mobile apps, cloud platforms and intelligent systems."
        canonicalPath="/technology"
      />
      <section className="py-16 md:py-24 bg-slate-950/20 border-t border-slate-900 max-w-7xl mx-auto px-6">
        <div className="space-y-10">
          <div className="text-center space-y-2">
            <span className="text-xs font-mono font-bold text-cyan-400 uppercase tracking-widest">BUILT WITH MODERN TECHNOLOGY</span>
            <h1 className="text-3xl font-extrabold font-sans text-slate-100 tracking-tight sm:text-4xl">Our Tech Stack</h1>
            <p className="text-xs sm:text-sm text-slate-400 max-w-xl mx-auto">We avoid template scripts. Every ecosystem application is programmed directly inside state-of-the-art frameworks to guarantee zero latency.</p>
          </div>
          <div className="flex flex-wrap items-center justify-center gap-3.5 max-w-4xl mx-auto">
            {[
              ...
              <div key={idx} className={`px-4 py-2 bg-slate-950 border rounded-2xl text-xs font-mono font-bold transition-all duration-300 hover:scale-105 hover:shadow-[0_0_15px_rgba(34,211,238,0.1)] ${tech.color}`}>{tech.name}</div>
            ))}
          </div>
        </div>
      </section>
